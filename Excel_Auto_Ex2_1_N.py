...
def data_copy():
    ...
    #print("max_r",max_r)
    list = ['I', 'J', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S']  # コピー先のセルが複数＜MA=10個＞用意
    for cntr in range(3, max_r+1):  # 3行目からスタート
        in_dat = sheet['I' + str(cntr)].value  # コピー元のセルが記載されている
        out_dat = sheet['J' + str(cntr)].value  # コピー先のセルが記載されている
        out_dat2 = sheet['K' + str(cntr)].value  # コピー先が２つ以上あるか確認
        #print("list2:",list[2])
        if out_dat2:  # コピー先が複数列ある！
            for rets in range(2,10+1):#S列まで確認！
                out_data_sq =sheet[list[rets]+str(cntr)].value#ここ相当苦労した！リストのときは"'"が不要
                #out_data_sq = sheet['K12'].value
                if out_data_sq:  # コピー先が複数列ある！
                    print()
                    data_cell_cpoy = data_cell_cpoy + "\nsheet_out['" + out_data_sq + "'""].value" + "=sheet_in['" + in_dat + "'""].value"
                else:
                    break
